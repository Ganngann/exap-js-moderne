export default `
<div class="slider fullsize">
  <div class="slides">

    <ul style="width: 500%" class="image-list">
    </ul>

</div>

<!-- MENU DE GAUCHE Photos -->
<div class="menu">
  <div class="slider-menu">
    <h1>Titre</h1>
    <ul class="slides">

      <li>
        <a href="#0" class="">
          <img src="slides/image1.jpg" alt="A cat game">
        </a>
      </li>

      <li>
        <a href="#1" class="">
          <img src="slides/image2.jpg" alt="Tatoo &amp; cat">
        </a>
      </li>

      <li>
        <a href="#2" class="">
          <img src="slides/image3.jpg" alt="Tatoo &amp; cat">
        </a>
      </li>

      <li>
        <a href="#3" class="">
          <img src="slides/image4.jpg" alt="Tatoo &amp; cat">
        </a>
      </li>

      <li>
        <a href="#4" class="">
          <img src="slides/image5.jpg" alt="Tatoo &amp; cat">
        </a>
      </li>
    </ul>
  </div>
</div>
<!-- /MENU DE GAUCHE Photos -->

<!-- NAVIGUATION -->
<div class="navigation">
  <div>
    <ul class="navigation">
      <!-- Bouton Précédent -->
      <li class="previous">
        <a href="#"><i class="material-icons">fast_rewind</i></a>
      </li>
      <!-- /Bouton Précédent -->
      <!-- Bouton Pause -->
      <li class="stop active">
        <a href="#"><i class="material-icons">pause_circle_filled</i></a>
      </li>
      <!-- /Bouton Pause -->
      <!-- Bouton Play -->
      <li class="play">
        <a href="#"><i class="material-icons">play_circle_filled</i></a>
      </li>
      <!-- /Bouton Play -->
      <!-- Bouton Suivant -->
      <li class="next">
        <a href="#"><i class="material-icons">fast_forward</i></a>
      </li>
      <!-- /Bouton Suivant -->
    </ul>
  </div>
</div>
<!-- /NAVIGUATION -->

</div>
</div>
`;
